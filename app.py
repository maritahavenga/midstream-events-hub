...
def c_a(n):
 ...
 return "Afrikaans "+("Eerste Addisionele Taal" if "eat" in n or "eerste" in n else "Hooftaal") if "eat" in n or "ht" in n or "hooftaal" in n else n.capitalize()
...
if not df.empty:
 ...
 for _,r in df.iterrows():
  ...
  if not cm or (sa and c_a(n) not in sa): continue
 ...
 h="<style>.card{background:white;padding:10px;border-radius:10px;border-left:8px solid #800000;margin-bottom:8px;box-shadow:0 2px 5px rgba(0,0,0,0.1);font-family:sans-serif;}.title{color:#800000;font-weight:bold;margin-bottom:5px;}.btn{background:#800000;color:white!important;padding:4px 8px;border-radius:5px;text-decoration:none;font-size:0.7rem;display:inline-block;margin:4px 4px 0 0;}.nt{background:#f0f7f7;padding:5px;margin-top:5px;border-radius:5px;font-size:0.75rem;}</style>"
 for i in res:
  r,ds=i['r'],i['dd']

...
def load_fresh():
    ...
    return df.sort_values(by='dt_fixed', ascending=True), now, datetime.now(SA_TIME)
...
try:
    ...
    with c[2]:
        sel_ages = st.multiselect("Age Group:", unique_ages, key="age_sel")
    ...
    if df.empty:
        st.markdown(f'<div class="no-data"><h3>📭 No information currently</h3><p>Try switching to "All Upcoming" view or changing search terms.</p></div>', unsafe_allow_html=True)
    else:

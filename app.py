...
if not df.empty:
 ...
 with c3:
  # ONS LOGIKA: Bepaal watter opsies om te WYS, maar hou die bron-lys dieselfde
  all_opts = ["Gr 1","Gr 2","Gr 3","Gr 4","Gr 5","Gr 6","Gr 7","U7","U8","U9","U10","U11","U12","U13"]
  if sc == ["Sport"]: disp_opts = [o for o in all_opts if "U" in o]
  elif sc and "Sport" not in str(sc): disp_opts = [o for o in all_opts if "Gr" in o]
  else: disp_opts = all_opts
  
  # Gebruik 'key' sodat Streamlit die waarde onthou
  sg = st.multiselect("Age Group", options=disp_opts, key="age_filter")

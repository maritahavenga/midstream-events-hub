...
def format_group_final(age_val, team_val):
    """Sjurgiese skoonmaak: Verhoed blank bladsye en hanteer 11.0 en reekse."""
    try:
        # Maak skoon en verwyder .0
        ...
        # Reeks logika: Slegs as daar 'n '-' is
...
if not df_raw.empty:
    ...
    # 1. Standaardiseer kolomme om crashes te voorkom
    df = df.fillna("")
    
    # 2. Aktiwiteit
    df['activity_display'] = df.iloc[:, 3].astype(str).str.replace("Hokkie", "Hockey", case=False).str.replace("Netbal", "Netball", case=False).str.replace("Atletiek", "Athletics", case=False)
    
    # 3. Groep (Kolom E=4, Kolom L=11)

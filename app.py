...
st.markdown("""<style>.stApp{background:#008080}.block-container{padding-top:0rem;max-width:800px}.card{background:white;padding:15px;border-radius:15px;border-left:12px solid #800000;margin-bottom:5px;box-shadow:0 4px 10px rgba(0,0,0,0.2)}.t{color:#800000;font-weight:bold;font-size:1rem}.v{color:#800000;font-weight:bold;text-decoration:underline}.stButton button{width:100%;background:#800000!important;color:white!important;font-weight:bold;height:3em;font-size:0.6rem!important;padding:0px!important}.box{background:#f1f3f5;padding:10px;border-radius:10px;margin-top:5px;border-left:5px solid #008080;color:#333;font-size:0.8rem}label{color:white!important;font-weight:bold}</style>""", unsafe_allow_html=True)
...
try:
    ...
    for _, r in data.iterrows():
        ...
        if "cult" in cat: ic = "https://www.svgrepo.com/show/396263/drama-masks.svg"
        elif "acad" in cat: ic = "https://www.svgrepo.com/show/532363/graduation-cap.svg"
        ...
        bx = f'<div class="box">ℹ️ {nfo}</div>' if (nfo and nfo.lower()!='nan' and not is_l) else ""
        ...
        if pd.notna(r.get('Program Link')):
            with c1: st.link_button("📜 PROG", str(r.get('Program Link')))
        ...
        if is_l:
            with c4: st.link_button("ℹ️ INFO", nfo)

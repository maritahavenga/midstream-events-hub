...
if not df.empty:
 ...
 with c3:
  # STABIELE LYS: Bepaal wat om te wys, maar hou die 'options' bron dieselfde sodat dit nie reset nie
 ...
 for _,r in df.iterrows():
  ...
  res.append({'r':r,'dt':dt if pd.notnull(dt) else datetime.max.replace(tzinfo=None),'ft':ft,'dd':dt.strftime('%d %B %Y') if pd.notnull(dt) else rd})

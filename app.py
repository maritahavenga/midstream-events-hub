...
if df_raw.empty:
    ...
    if df.empty:
        ...
        for _, r in df.iterrows():
            cat_raw, sport_raw, age_raw, raw_dt, ven_raw = str(r.iloc[0]), str(r.iloc[1]), str(r.iloc[2]), str(r.iloc[3]), str(r.iloc[4])
            ...
            h += f"<div class='card'><div style='font-size:0.75rem; color:#666; text-transform:uppercase;'>{cat_raw}</div><div class='card-title'>{sport_raw} {age_raw}</div><div style='font-size:0.9rem; color:#008080;'>📅 {display_date}</div><div style='font-size:0.9rem; color:#444;'>📍 {ven_raw}</div>{btns}{extra}</div>"

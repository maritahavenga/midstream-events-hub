...
# URLS (Onthou om jou Newsletters GID op te dateer!)
...
NEWS_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSW1BP7Gds7hz04Gdrqrigq2SEVrUB_cmkkMo6Bh-4hci-YcjK3Ww9tVr7-GmKbWDPkCSwd0SLW2Ai8/pub?gid=0&single=true&output=csv"

def fix_drive_link(url):
    u = str(url).strip()
    if u.lower() in ["n/a", "na", "nan", ""]: return ""
...
def format_group_final(text):
    """Logika vir enige ouderdom-reeks (U7-U9) en N/A skoonmaak."""
    ...
    if len(nums) >= 2:
        return f"U{nums[0]}-U{nums[1]} {gender}{team}".strip()
    elif len(nums) == 1:
        return f"U{nums[0]}{team} {gender}".strip()
    return t

@st.cache_data(ttl=2)
def load_data(url, is_events=False):
    try:
        r = requests.get(f"{url}&cb={datetime.now().timestamp()}", timeout=10)
        df = pd.read_csv(io.StringIO(r.content.decode('utf-8')))
        if df.empty: return pd.DataFrame()
        
        # Maak N/A skoon in die hele dataframe
        df = df.replace(['N/A', 'n/a', 'NA', 'na', 'nan'], '', regex=True)
        
        if is_events:
            # Verwyder duplikate (keep last = die nuutste edit)
            df = df.drop_duplicates(subset=[df.columns[3], df.columns[4], df.columns[5], df.columns[6]], keep='last')
        return df
...
tab_events, tab_news = st.tabs(["🗓️ EVENTS", "📰 NEWSLETTERS"])

with tab_events:
    df_raw = load_data(EVENTS_URL, is_events=True)
    ...
    if not df_raw.empty:
        df = df_raw.copy()
        df['activity_display'] = df.iloc[:, 3].fillna("").astype(str).str.replace("Hokkie", "Hockey", case=False).str.replace("Netbal", "Netball", case=False)
        ...
        for _, r in df.iterrows():
            ...
            if ven_raw in ["N/A", "NA", ""]: ven_html = "📍 VENUE TBC"
            else:
            ...
            h += f"<div class='card'>{badge}<div class='card-title'>{r['activity_display']} {r['group_display']}</div><div class='info-row'>🗓️ {formatted_date}</div><div class='info-row'>{ven_html}</div><div style='display:block;'>{btns}</div></div>"
        
        components.html(h, height=1500, scrolling=True)

with tab_news:
    df_news = load_data(NEWS_URL)
    st.markdown("### 📰 Latest School Newsletters")
    if not df_news.empty:
        for _, n in df_news.iterrows():
            with st.container():
                col1, col2 = st.columns([3, 1])
                with col1:
                    st.markdown(f"**{n.iloc[1]}**")
                    st.caption(f"Published: {n.iloc[0]}")
                with col2:
                    st.link_button("VIEW PDF", n.iloc[2])
                st.divider()

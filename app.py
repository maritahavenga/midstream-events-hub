...
# Ons hou eers net by die Events URL soos jy gevra het
...
def format_group_final(text):
    """Logika wat ELKE nommer met 'n U vervang vir reekse soos U10-U13."""
    if not text or str(text).lower() in ["nan", "n/a", "na", ""]: return ""
    t = str(text).strip().upper()
    
    # 1. Bepaal geslag en span
    ...
    # 2. Vind alle nommers en sit 'n U voor elkeen
    # Hierdie vervang bv "10-13" met "U10-U13"
    t_fixed = re.sub(r'(\d+)', r'U\1', t)
    
    # 3. Verwyder die ou woorde soos "DOGTERS" of "O/" wat nou dalk dubbel is
    clean_label = t_fixed.replace("O/", "").replace("UNDER", "").replace("ONDER", "").replace("BOYS", "").replace("GIRLS", "").replace("SEUNS", "").replace("DOGTERS", "").strip()
    
    # 4. Bou die finale string
    return f"{clean_label} {gender}{team}".strip()
...
def load_data(url):
    try:
        ...
        # STRENG DUPLIKAAT FILTER (Kyk na Aktiwiteit, Groep en Datum)
        # As 'n onderwyser die inskrywing edit, hou ons net die heel laaste een (keep last)

...
def format_group_cleanly(text):
    """Die 'Spaghetti-Killer' logika: Dwing U12 Girls formaat."""
    if not text or str(text).lower() == 'nan': return ""
    t = str(text).strip().lower()
    
    # Kry die nommer (ouderdom)
    nums = re.findall(r'\d+', t)
    age = nums[0] if nums else ""
    
    # Bepaal geslag
    gender = ""
    if any(x in t for x in ["g", "girl", "dogter"]): gender = "Girls"
    elif any(x in t for x in ["b", "boy", "seun"]): gender = "Boys"
    
    # Bepaal span (A, B, C)
    team = ""
    if " a" in t or t.endswith("a"): team = " A"
    elif " b" in t or t.endswith("b"): team = " B"
    elif " c" in t or t.endswith("c"): team = " C"
    
    # BOU DIT VAN NUUTS AF: U[nommer][spasie][Geslag][Span]
    if age:
...
def load_data():
    try:
        ...
        # Pas nuwe logika toe
        df['category_display'] = df.iloc[:, 2].fillna("").astype(str)
        ...
        df['group_display'] = df.iloc[:, 4].apply(format_group_cleanly)
...
with st.container():
    ...
    search_q = st.text_input("🔍 Search:", placeholder="Search event...").lower().strip()
...
if df_raw.empty:
    ...
    if df.empty:
        st.write("<p style='text-align:center; padding:20px;'>No upcoming events.</p>", unsafe_allow_html=True)
    else:
        h = """<style>

...
search_q = st.text_input(
    "🔍 Search Activity or Age:",
    placeholder="e.g. athletics, u13"
).lower().strip()
...
if not raw_df.empty:
    ...
    # --- DATE FILTER (FIXED FOR ATHLETICS) ---
    if view == "Upcoming":
        df = raw_df[
        ...
        for idx, lbl in [(5, "PROGRAMME"), (6, "TEAM"), (7, "CONFIRM"), (8, "INFORMATION")]:
            ...
            if link:
                ...
                if lbl == "PROGRAMME":
                    ...
                    other_btns.append(
                        f'<a href="{url}" target="_blank" class="btn">{lbl}</a>'
                    )
            else:
                if lbl == "TEAM":
                    team_text = html.escape(val)
                elif lbl == "INFORMATION":
                    note_text = html.escape(val)

        team_html = (
            f'<div class="team-box"><b>TEAMS:</b><br>{team_text}</div>'
            if team_text else ""
        )

        btns_html = (
            f'<div class="btn-row">{" ".join(other_btns)}</div>'
            if other_btns else ""
        )

        note_html = (
            f'<div class="box"><b>Note:</b><br>{note_text}</div>'
            if note_text else ""
        )

        prog_html = (
            f'<div class="prog-container">'
            f'<a href="{prog_link}" target="_blank" class="btn">PROGRAMME</a>'
            f'</div>'
            if prog_link else ""
        )

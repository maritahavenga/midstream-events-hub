...
st.markdown("""<style>.stApp{background:#008080}.block-container{padding-top:0rem;max-width:800px}.card{background:white;padding:20px;border-radius:15px;border-left:12px solid #800000;margin-bottom:5px;box-shadow:0 4px 10px rgba(0,0,0,0.2)}.t{color:#800000;font-weight:bold;font-size:1.1rem}.v{color:#800000;font-weight:bold;text-decoration:underline}.stButton button{width:100%;background:#800000!important;color:white!important;font-weight:bold;height:3em;font-size:0.7rem!important;padding:0px!important}.box{background:#f1f3f5;padding:10px;border-radius:10px;margin-top:10px;border-left:5px solid #008080;color:#333;font-size:0.85rem}label{color:white!important;font-weight:bold}</style>""", unsafe_allow_html=True)
...
try:
    ...
    for i, r in df.iterrows():
        ...
        st.markdown(f'''<div class="card"><img src="{ic}" width="45" style="float:right"><div>📅 {r.get("Date","TBA")}</div><div class="t">{r.get("Event","Event")}</div><div style="margin-top:5px">📍 <a href="{mu}" target="_blank" class="v">{v}</a></div>{bx}</div>''', unsafe_allow_html=True)

...
try:
    ...
    for i, r in df.iterrows():
        ...
        # Check if Information is a Link or Text
        ...
        c1, c2 = st.columns(2)
        if pd.notna(r.get('Program Link')): with c1: st.link_button("📜 PROGRAM", str(r.get('Program Link')))
        if pd.notna(r.get('Team/Cast Link')): with c2: st.link_button("🏃 TEAM LIST", str(r.get('Team/Cast Link')))
        
        c3, c4 = st.columns(2)
        if pd.notna(r.get('Transport/Sign-up')): with c3: st.link_button("🚌 BUS SIGN-UP", str(r.get('Transport/Sign-up')))
        if is_link: with c4: st.link_button("ℹ️ VIEW LETTER", info)

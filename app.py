...
# Die URL van jou gepubliseerde Google Sheet
...
def fix_drive_link(url):
    """Dwing Google Drive skakels na 'n publieke viewer formaat met sharing permissions."""
    u = str(url).strip()
...
def clean_text(text, is_group=False, is_category=False):
    """Maak teks skoon en vertaal Afrikaans na Engels vir die vertoon."""
...
def load_data():
    try:
        ...
        # Dwing datum kolom na regte datum formaat
...
with st.container():
    ...
    search_q = st.text_input("🔍 Search Events:", placeholder="Search by activity, group or venue...").lower().strip()
...
if df_raw.empty:
    ...
    if 'dt_fixed' in df.columns:
        # DIE POOF LOGIKA: Gooi slegte datums weg en wys net vandag of later

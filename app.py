...
if not df_raw.empty:
    ...
    for item in df_filtered:
        r = item['data']
        # AS DIT AFRIKAANS IS, GEBRUIK 'DOCUMENT'
        is_acad_related = "academic" in str(r.iloc[2]).lower() or "afrikaans" in str(r.iloc[3]).lower()
        
        t_act = translate_term(str(r.iloc[3]), str(r.iloc[3]))
        full_title = f"{t_act} {'Gr ' if is_acad_related else 'U'}{clean_val(r.iloc[11])} {translate_term(clean_val(r.iloc[4]), str(r.iloc[3]))}"
        ...
        btns = ""
        # Knop naam verander na 'Document' vir enige iets wat 'Afrikaans' is
        ...
        h += f"""<div class='card'>
            ...
            {note_html if 'note_html' in locals() else ''}<div class='btn-box'>{btns}</div>
        </div>"""
        note_html = ""

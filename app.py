    ...
    with st.container():
        ...
        with c2:
            ...
            for o in o_r:
                ...
                elif re.search(r'(?i)\b(EAT|Afrikaans EAT|HT|Afrikaans HT)\b', o): cl_o.add("Afrikaans Eerste Addisionele Taal" if "eat" in lo else "Afrikaans Hooftaal")
                else: cl_o.add(o)
            s_act = st.multiselect("Activity", sorted(list(cl_o)))
        with c3:
            al = ["U7","U8","U9","U10","U11","U12","U13"] if s_cat == ["Sport"] else ["Gr 1","Gr 2","Gr 3","Gr 4","Gr 5","Gr 6","Gr 7"]
            if not s_cat or len(s_cat) > 1: al = ["Gr 1","Gr 2","Gr 3","Gr 4","Gr 5","Gr 6","Gr 7","U7","U8","U9","U10","U11","U12","U13"]
            s_age = st.multiselect("Age Group", al)
